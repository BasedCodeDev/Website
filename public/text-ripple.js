(function (global) {
  ...
  function getTextNodes(root) {
    ...
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        ...
        if (parent.classList && parent.classList.contains("ripple-char")) return NodeFilter.FILTER_REJECT;
  ...
  function normalizeTextForSlots(node) {
    ...
    if (!node.previousSibling) value = value.replace(/^\s+/, "");
    if (!node.nextSibling) value = value.replace(/\s+$/, "");
    return value;
  }
  ...
  function buildRippleModel(element) {
    ensureRippleStyles();
    if (element._rippleModel) return element._rippleModel;
  ...
  function scrambleReveal(element, options = {}) {
    ...
    if (!preserveText) writeFrameToSlots(slots, blankedText);
      ...
      function tick(now) {
        ...
        if (rawProgress > 0 && rawProgress < 1) {
          ...
          for (let index = 0; index < mutations; index += 1) {
            ...
            if (target < 0 || target >= originalText.length || /\s/.test(originalText[target])) continue;
            ...
            const nextChar = Math.random() < resolveBias ? originalText[target] : randomGlyph();
  ...
  function cancelAnimations() {
    for (const animation of activeAnimations) animation.cancelled = true;
  }
  ...
  function startRippleInterval(intervalSeconds, speedMultiplier = 1, root = document) {
    ...
    if (!Number.isFinite(delayMs) || delayMs < 0) throw new Error("intervalSeconds must be a non-negative number.");
